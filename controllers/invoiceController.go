...
func GetInvoice() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		log.Printf("Fetching invoice with ID: %s", invoiceId)
		...
		if err != nil {
			if err == mongo.ErrNoDocuments {
				...
				log.Printf("Error occurred while fetching invoice: %v", err) // Log detailed error
		...
		allOrderItems, err := ItemsByOrder(invoice.Order_id)
		if err != nil {
			log.Printf("Error occurred while fetching order items: %v", err) // Log detailed error
		...
		if len(allOrderItems) == 0 {
			c.JSON(http.StatusNotFound, gin.H{"error": "No order items found for the specified invoice"})
			return
		}
