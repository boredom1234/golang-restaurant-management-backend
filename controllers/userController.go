...
func GetUsers() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// Aggregate Pipeline
		matchStage := bson.D{{"$match", bson.D{{}}}}    // Adjust if you need any filters
		countStage := bson.D{{"$count", "total_count"}} // Count the total number of documents
		sliceStage := bson.D{{"$project", bson.D{
			{"_id", 0},
			{"total_count", 1},
			{"user_items", bson.D{{"$slice", []interface{}{"$data", startIndex, recordPerPage}}}},
		}}}

		// First Aggregate: Get total count
		totalCountResult, err := userCollection.Aggregate(ctx, mongo.Pipeline{matchStage, countStage})
		if err != nil {
		...
		if err := totalCountResult.All(ctx, &countResult); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while counting users"})
			return
		}
		totalCount := 0
		if len(countResult) > 0 {
			totalCount = int(countResult[0]["total_count"].(int32))
		}

		// Second Aggregate: Get paginated users
		paginatedResult, err := userCollection.Aggregate(ctx, mongo.Pipeline{matchStage, sliceStage})
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while listing users"})
			return
		}

		var paginatedUsers []bson.M
		if err := paginatedResult.All(ctx, &paginatedUsers); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while listing users"})
			return
		}

		if len(paginatedUsers) == 0 {
			c.JSON(http.StatusOK, gin.H{"total_count": totalCount, "user_items": []interface{}{}})
			return
		}

		c.JSON(http.StatusOK, gin.H{"total_count": totalCount, "user_items": paginatedUsers[0]["user_items"]})
	}
}
