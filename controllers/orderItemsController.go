...
// 	projectStage := bson.D{
// 		{"$project", bson.D{
// 			{"id", 0}, // Exclude `_id` if not needed
// 			{"amount", "$food.price"},
// 			{"total_count", 1},
// 			{"food_name", "$food.name"},
// 			{"food_image", "$food.food_image"},
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	projectStage := bson.D{
		{"$project", bson.D{
			{"_id", 0},                // Exclude `_id`
			{"amount", "$food.price"}, // Include amount
			{"total_count", 1},
			{"food_name", "$food.name"},
			{"food_image", "$food.food_image"},
			{"table_number", "$table.table_number"},
	...
	return func(c *gin.Context) {
		...
		if err != nil {
			if err == mongo.ErrNoDocuments {
				...
				c.JSON(http.StatusInternalServerError, gin.H{"error": "Error occurred while listing ordered item: " + err.Error()})
			}
			return
		}
